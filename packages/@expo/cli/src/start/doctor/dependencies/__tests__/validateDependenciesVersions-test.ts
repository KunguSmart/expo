...
describe('getVersionedDependenciesAsync (TV)', () => {
  ...
  it('rewrites the expected version for react-native in TV projects to an npm-alias spec', async () => {
    ...
    // package, so the version reads as a plain version string.
    vol.fromJSON(
      {
  ...
  it('does not flag react-native when the TV project is at the bundled tvos version', async () => {
    vol.fromJSON(
      {
        'node_modules/expo/package.json': JSON.stringify({ version: '55.0.0' }),
        'node_modules/react-native/package.json': JSON.stringify({
    ...
    const exp = { sdkVersion: '55.0.0' };
    const pkg = {
      dependencies: { 'react-native': 'npm:react-native-tvos@0.85.3-0' },
    };
